...
private extension TickerView {
    func panel(for symbol: String) -> DotGrid {
        PanelRenderer.render(symbol: symbol, state: states[symbol] ?? .loading, shares: shares[symbol] ?? 0, showsValueRow: showsValueRow, logo: logos[symbol])
    }
}
